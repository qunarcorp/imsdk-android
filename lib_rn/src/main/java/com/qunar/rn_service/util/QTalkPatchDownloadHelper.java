    ...
    public static boolean downloadFullPackageAndCheck(String fullpackageUrl,  // 资源下载路径 md5文件
        ...
        if (downloadUrl.startsWith("https://") || downloadUrl.startsWith("http://")) {
            try {
                ...
                if(QTalkPatchDownloadHelper.saveResponse2file(response, path, filename) &&
                        QTalkPatchDownloadHelper.checkBundleMD5(path, filename, fullpackageMd5) &&
                        QTalkPatchDownloadHelper.unzipBundle(path, filename) &&
                        QTalkPatchDownloadHelper.renameFile(path, bundleName, finalBundleFile)){
    ...
    public static boolean downloadPatchAndCheck(String patchUrl,
        ...
        if (downloadUrl.startsWith("https://") || downloadUrl.startsWith("http://")) {
            try {
                ...
                if(!QTalkPatchDownloadHelper.saveResponse2file(response, path, patch)){
                    // save patch
                    return false;
                }

                byte[] patchBytes = FileHelper.toByteArray(patchFilePath);
                if(QTalkPatchDownloadHelper.checkBundleMD5(path, patch, patchMd5) && // check patch md5
                        PatchHelper.patchByBytes(originFileBytes, patchBytes, tempDestFilePath) && //patch
                        QTalkPatchDownloadHelper.checkBundleMD5(destPath, tempDestFile, fullMd5) && // patch bundle md5
                        QTalkPatchDownloadHelper.renameFile(destPath, tempDestFile, destFile) // rename patch bundle file
                        ){
