...
public class ChatroomInfoPresenter implements IChatroomInfoPresenter, IMNotificaitonCenter.NotificationCenterDelegate {
    ...
    public void showMembers(boolean onlyFromDB) {
        ...
        for (GroupMember member : groupMemberList) {
            if (QtalkStringUtils.userId2Jid(com.qunar.im.protobuf.common.CurrentPreference.getInstance().getPreferenceUserId()).equals(
                    member.getMemberId())) {
    ...
    public void didReceivedNotification(String key, Object... args) {
        switch (key) {
            ...
            case QtalkEvent.Invite_User:
                showMembers(false);
//                connectionUtil.getMembersAfterJoin(chatRoomInfoView.getRoomId());
                break;
            case QtalkEvent.Del_Muc_Register:
                String deleteId = (String) args[0];
                if (deleteId != null && deleteId.equals(chatRoomInfoView.getRoomId())) {
                    chatRoomInfoView.closeActivity();
                    return;
                }
                showMembers(false);
//                connectionUtil.getMembersAfterJoin(chatRoomInfoView.getRoomId());
            ...
            case QtalkEvent.Destory_Muc:
                String destory = (String) args[0];
                if (destory != null && destory.equals(chatRoomInfoView.getRoomId())) {
                    chatRoomInfoView.closeActivity();
