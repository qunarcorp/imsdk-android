...
//        Protocol.uploadSelfImei(uuid);
    ...
    public void getPushState() {
        Protocol.getPushState(new ProtocolCallback.UnitCallback<String>() {
            @Override
            public void onCompleted(String s) {
                Map<String, Object> map = JsonUtils.getGson().fromJson(s, Map.class);
                if(map != null && map.containsKey("ret") && Boolean.valueOf(map.get("ret").toString())){
                    Map<String, Object> inMap = (Map<String, Object>) map.get("data");
                    if(inMap.containsKey("state")){
                        com.qunar.im.protobuf.common.CurrentPreference.getInstance().setOfflinePush(Boolean.valueOf(inMap.get("state").toString()));
                    }
                }
            }
            ...
            @Override
            public void onFailure(String errMsg) {
                Logger.i("getPushState onFailure");
                com.qunar.im.protobuf.common.CurrentPreference.getInstance().setOfflinePush(false);
            }
        });
    }
