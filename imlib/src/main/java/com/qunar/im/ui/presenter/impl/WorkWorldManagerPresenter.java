...
//        }
        ...
        WorkWorldItem item = mView.getLastItem();
        if (item == null) {
            return;
        }
...
//                mView.showNewData(workWorldResponse.getData().getNewPost());
//            }
//
//            @Override
...
//        int postType = WorkWorldItemState.normal;
        ...
        HttpUtil.refreshWorkWorldV2(listSize, childListSize, postType, owner, ownerHost, Long.parseLong(item.getCreateTime()), new ProtocolCallback.UnitCallback<WorkWorldResponse>() {
            @Override
            public void onCompleted(WorkWorldResponse workWorldResponse) {
                mView.showMoreData(workWorldResponse.getData().getNewPost());
            }
            ...
            public void onFailure(String errMsg) {
                ...
                if (isUser) {
                    list = ConnectionUtil.getInstance().selectHistoryWorkWorldItem(size, mView.getListCount(), searchId);
                } else {
                    list = ConnectionUtil.getInstance().selectHistoryWorkWorldItem(size, mView.getListCount());
                }
    ...
    public void deleteWorkWorldItem(WorkWorldItem item) {
        HttpUtil.deleteWorkWorldItem(item.getUuid(), new ProtocolCallback.UnitCallback<WorkWorldDeleteResponse>() {
            ...
            public void onCompleted(WorkWorldDeleteResponse deleteWorkWorldItem) {
                mView.removeWorkWorldItem(deleteWorkWorldItem);
            }
