...
public class SmartTabStrip extends LinearLayout {
  ...
  private void drawDecoration(Canvas canvas) {
    ...
    if (tabCount > 0) {
      ...
      int left;
      int right;
      boolean isSetWidth=false;
      int diff = 0;
      if (isLayoutRtl) {
        left = selectedEnd;
        right = selectedStart;
        if(indicatorWidth>0&&indicatorWidth<left-right)
        {
          isSetWidth = true;
          diff = (left - right - indicatorWidth)/2;
          right = right +diff;
          left = left -diff;
        }
      } else {
        left = selectedStart;
        right = selectedEnd;
        if(indicatorWidth>0&&indicatorWidth<right-left)
        {
          isSetWidth = true;
          diff = (right - left - indicatorWidth)/2;
          left = left +diff;
          right = right -diff;
        }
      }
