...
import com.qunar.im.ui.view.camera.state.CameraMachine;
import com.qunar.im.ui.view.camera.utils.FileUtil;

import java.io.IOException;


public class QCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
    ...
    //回调监听
    private QCameraListener qCameraLisenter;
    ...
    public QCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        //get AttributeSet
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.QCameraView, defStyleAttr, 0);
        iconSize = a.getDimensionPixelSize(R.styleable.QCameraView_iconSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 35, getResources().getDisplayMetrics()));
        iconMargin = a.getDimensionPixelSize(R.styleable.QCameraView_iconMargin, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
        iconSrc = a.getResourceId(R.styleable.QCameraView_iconSrc, R.drawable.atom_ui_ic_camera);
        iconLeft = a.getResourceId(R.styleable.QCameraView_iconLeft, 0);
        iconRight = a.getResourceId(R.styleable.QCameraView_iconRight, 0);
        duration = a.getInteger(R.styleable.QCameraView_duration_max, 10 * 1000);       //没设置默认为10s
        a.recycle();
        initData();
        initView();
    }
    ...
    private void initData() {
        layout_width = Utils.getScreenWidth(mContext);
        //缩放梯度
...
//                if (qCameraLisenter != null) {
//                    qCameraLisenter.quit();
//                }
//            }
//        });
    ...
    public void onResume() {
        LogUtil.i("QCameraView onResume");
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        LogUtil.i("QCameraView SurfaceCreated");
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(QCameraView.this);
            }
        }.start();
    }
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        LogUtil.i("QCameraView SurfaceDestroyed");
...
//                    Log.i(TAG, "result = " + (result - firstTouchLength));
    ...
    public void setQCameraLisenter(QCameraListener qCameraLisenter) {
        this.qCameraLisenter = qCameraLisenter;
    }
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                machine.start(mVideoView.getHolder(), screenProp);
                if (qCameraLisenter != null) {
                    qCameraLisenter.recordSuccess(videoUrl, firstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
                if (qCameraLisenter != null) {
                    qCameraLisenter.captureSuccess(captureBitmap);
                }
                break;
            case TYPE_SHORT:
                break;
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        QCameraView.this.firstFrame = firstFrame;
...
//            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    ...
    private void setFlashRes() {
        ...
        try{
            ...
            machine.flash(p.getFlashMode());//获取闪光灯的状态
        }catch (Exception e){
            Toast.makeText(mContext,mContext.getString(R.string.atom_ui_tip_operation_failed),Toast.LENGTH_LONG).show();
        }
//        switch (type_flash) {
